...
void mock_check_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void mock_check_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "mock_check";
ctx->componentMemory[coffset].signalStart = soffset;
ctx->componentMemory[coffset].inputCounter = 1;
ctx->componentMemory[coffset].componentName = componentName;
ctx->componentMemory[coffset].idFather = componentFather;
...
int cmp_index_ref_load = -1;
{
std::string new_cmp_name = "x_0_is_equal";
...
PFrElement aux_dest = &signalValues[mySignalStart + 2];
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 1],&signalValues[mySignalStart + 1]); // line circom 11
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
// load src
Fr_mul(&expaux[0],&signalValues[mySignalStart + 2],&circuitConstants[2]); // line circom 12
// end load src
Fr_copy(aux_dest,&expaux[0]);
}
{
...
mock_check_2_create(1,0,ctx,"main",0);
mock_check_2_run(0,ctx);
}
